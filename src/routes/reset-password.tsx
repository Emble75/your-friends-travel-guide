...
export const Route = createFileRoute("/reset-password")({
  head: () => ({
    meta: [{ title: "Neues Passwort – Turi" }],
  }),
...
function ResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "Passwort konnte nicht geändert werden"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="flex min-h-screen flex-col justify-center bg-background">
      <div className="app-shell py-10">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-5 text-3xl font-bold">Neues Passwort</h1>
        ...
        >
          ...
          <Button type="submit" disabled={loading} className="h-12 w-full rounded-2xl text-base">
            {loading ? "Moment…" : "Passwort speichern"}
          </Button>
        </form>
      </div>
    </main>
